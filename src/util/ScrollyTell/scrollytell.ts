...
export class Story {
    ...
    private readonly chart?: HTMLElement | null;
    private readonly config: StoryConfig;
    private readonly container: HTMLElement | null;
    private developerHudCanvas?: HTMLCanvasElement;
    private developerHudContext?: CanvasRenderingContext2D | null;
    ...
    public constructor(config: StoryConfig) {
        this.config = config;
        ...
        if (config.chartSelector) {
            this.chart = document.querySelector(config.chartSelector) as HTMLElement;
        }
        ...
        if (config.fullsizeChart && this.chart) {
            ...
            if (config.segmentSelector) {
                const segments: NodeListOf<HTMLElement> = document.querySelectorAll(
    ...
    private renderDeveloperHud(containerBox: ClientRect) {
        ...
        // these are redundant but typescrip wanted both
        if (!ctx) {
            return;
        }
        if (!this.developerHudCanvas) {
        ...
        const { width } = this.developerHudCanvas;
        const { height } = this.developerHudCanvas;
